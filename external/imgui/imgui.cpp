...
int ImTextCharFromUtf8(unsigned int* out_char, const char* in_text,
  ...
  if ((*str & 0xf8) == 0xf0) {
    *out_char =
    ...
    // If codepoint does not fit in ImWchar, use replacement character U+FFFD
    // instead
...
bool ImGuiListClipper::Step() {
  ...
  if (StepNo ==
      2)  // Step 2: empty step only required if an explicit items_height was
...
static void ImGui::ErrorCheckNewFrameSanityChecks() {
  ...
  // #define IM_ASSERT(EXPR)   SomeCode(EXPR); SomeMoreCode(); // Wrong! #define
  // IM_ASSERT(EXPR)   do { SomeCode(EXPR); SomeMoreCode(); } while (0)   //
  // Correct!
  if (true)
    IM_ASSERT(1);
  else
...
// default.
static float CalcScrollSnap(float target, float snap_min, float snap_max,
...
void ImGui::SetScrollHereX(float center_x_ratio) {
  ...
  const float snap_x_max = window->DC.CursorStartPos.x + window->ContentSize.x +
                           window->WindowPadding.x;
  target_x = CalcScrollSnap(target_x, snap_x_min, snap_x_max, snap_x_threshold,
                            center_x_ratio);
...
void ImGui::SetScrollHereY(float center_y_ratio) {
  ...
  const float snap_y_max = window->DC.CursorStartPos.y + window->ContentSize.y +
                           window->WindowPadding.y;
  target_y = CalcScrollSnap(target_y, snap_y_min, snap_y_max, snap_y_threshold,
                            center_y_ratio);
...
bool ImGui::OpenPopupContextItem(const char* str_id,
...
void ImGui::ShowMetricsWindow(bool* p_open) {
  ...
  if (ImGui::TreeNode("Settings")) {
    ...
    if (ImGui::TreeNode("SettingsIniData",
      ...
      char* buf =
          (char*)(void*)(g.SettingsIniData.Buf.Data ? g.SettingsIniData.Buf.Data
                                                    : "");
