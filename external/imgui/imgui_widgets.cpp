...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min,
  ...
  if (is_logarithmic) {
    ...
    if (is_decimal) {
      ...
      // For integer values we want the clicking position to match the grab box
      // so we round above This code is carefully tuned to work with large
      // values (e.g. high ranges of U64) while preserving this property..
  ...
  if (g.ActiveId == id) {
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
        ...
        clicked_t = ScaleRatioFromValueT<TYPE, FLOATTYPE>(
